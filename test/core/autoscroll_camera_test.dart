...
void main() {
  test('V0AutoscrollCamera: player past threshold pulls target forward', () {
    final movement = V0MovementTuningDerived.from(const V0MovementTuning(), tickHz: 60);
    final tuning = V0CameraTuningDerived.from(const V0CameraTuning(), movement: movement);
  ...
  test('GameCore: falling behind camera ends run and emits RunEndedEvent once', () {
    final core = GameCore(
      seed: 1,
      tickHz: 20,
    ...
    for (var i = 0; i < 200; i += 1) {
      ...
      for (final e in core.drainEvents()) {
        if (e is RunEndedEvent) {
          expect(ended, isNull, reason: 'RunEndedEvent should be emitted once');
          ended = e;
        }
      }
