...
void main() {
  ...
  test('save then load preserves data and writes deterministic ordering', () async {
    final root = Directory.systemTemp.createTempSync('prefab_store_roundtrip_');
    try {
      final input = PrefabData(
        ...
        prefabs: const [
          PrefabDef(
            ...
            anchorYPx: 5,
            colliders: [
              PrefabColliderDef(offsetX: 0, offsetY: 0, width: 10, height: 12),
            ],
            tags: ['b', 'a'],
          ),
          PrefabDef(
      ...
      expect(
        loaded.platformModules.map((m) => m.id),
        ['module_a', 'module_b'],
      );
      ...
      expect(prefabJson.indexOf('"a_slice"'), lessThan(prefabJson.indexOf('"z_slice"')));
      expect(tileJson.indexOf('"module_a"'), lessThan(tileJson.indexOf('"module_b"')));
    } finally {
      root.deleteSync(recursive: true);
    }
  });
