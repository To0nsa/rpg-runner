...
Future<void> main() => integrationDriver(
  responseDataCallback: (data) async {
    ...
    if (revision.exitCode != 0 || status.exitCode != 0) {
      throw StateError('Could not read acceptance repository identity.');
    }
    ...
    await writeResponseData(
      report,
      testOutputFilename: 'windows_chunk_playtest_phase6',
      destinationDirectory: p.join(repositoryRoot.path, '.tmp'),
    );
  },
);
...
Directory _findRepositoryRoot() {
  ...
  while (true) {
    if (Directory(p.join(current.path, '.git')).existsSync()) return current;
