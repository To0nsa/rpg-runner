...
List<ValidationIssue> validateChunkV2StagingDocument(
  ...
  for (var chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
    ...
    var directGeometryAccepted = true;
    for (final shape in chunk.collisionShapes) {
      final shapePath = '${sourcePath ?? chunk.chunkKey}:${shape.shapeId}';
      ...
      for (final diagnostic in review.diagnostics) {
        ...
        if (terrainDiagnosticIsBlocking(diagnostic)) {
          directGeometryAccepted = false;
        }
      }
    }
    if (directGeometryAccepted) {
      try {
        const TerrainCompiler().compile(
          chunk.collisionShapes.map(
            (shape) => TerrainSourceCoreAdapter.toPolygonInput(
              shape: shape,
              sourcePath: '${sourcePath ?? chunk.chunkKey}:${shape.shapeId}',
              chunkIndex: chunkIndex,
              chunkKey: chunk.chunkKey,
            ),
          ),
