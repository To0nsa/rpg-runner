import 'package:meta/meta.dart';

import '../workspace/editor_workspace.dart';

/// Immutable command envelope sent from UI/session to a domain plugin.
///
/// [kind] is plugin-defined and should be stable across one editor version.
/// [payload] carries command arguments as JSON-like scalar/map/list values or
/// immutable typed snapshots owned by the caller.
@immutable
class AuthoringCommand {
  AuthoringCommand({
    required this.kind,
    Map<String, Object?> payload = const <String, Object?>{},
  }) : payload = Map<String, Object?>.unmodifiable(
         payload.map(
           (key, value) =>
               MapEntry<String, Object?>(key, _freezePayloadValue(value)),
         ),
       );

  final String kind;
  final Map<String, Object?> payload;

  static Object? _freezePayloadValue(Object? value) {
    if (value is Map<Object?, Object?>) {
      return Map<Object?, Object?>.unmodifiable(
        value.map(
          (key, nestedValue) =>
              MapEntry<Object?, Object?>(key, _freezePayloadValue(nestedValue)),
        ),
      );
    }
    if (value is List<Object?>) {
      return List<Object?>.unmodifiable(value.map(_freezePayloadValue));
    }
    return value;
  }
}

/// Plugin-owned immutable source-of-truth snapshot for one authoring domain.
///
/// Implementations should treat instances as value snapshots suitable for
/// undo/redo history and deterministic recomputation.
abstract class AuthoringDocument {
  const AuthoringDocument();
}

/// UI-facing projection derived from an [AuthoringDocument].
///
/// This can be optimized for editing/rendering concerns, but it must remain
/// deterministic for identical document input.
abstract class EditableScene {
  const EditableScene();
}

/// Validation levels used by domain plugins.
enum ValidationSeverity { info, warning, error }

/// Independently admitted authoring actions; saving a design need not make it
/// runnable or ready for generated game content.
enum AuthoringOperation { save, play, build }

/// One plugin validation finding for the current document state.
///
/// [code] should remain stable for a specific issue type so UI/tests can match
/// without parsing [message]. [sourcePath] is optional and points to the file
/// most closely associated with the issue when known. [ownerKey] identifies
/// the repository object that must be edited to resolve the issue when the
/// validating domain has a stable owner identity.
@immutable
class ValidationIssue {
  const ValidationIssue({
    required this.severity,
    required this.code,
    required this.message,
    this.sourcePath,
    this.ownerKey,
    this.placementKey,
    this.shapeId,
    this.elementIndex,
    this.elementId,
    this.fieldKey,
    this.blockingOperations = const <AuthoringOperation>{
      AuthoringOperation.save,
      AuthoringOperation.play,
      AuthoringOperation.build,
    },
  });

  final ValidationSeverity severity;
  final String code;
  final String message;
  final String? sourcePath;

  /// Optional stable key of the source object that owns this finding.
  final String? ownerKey;

  /// Optional stable placed-prefab identity for expanded collision findings.
  final String? placementKey;

  /// Optional polygon owner-local shape identity for diagnostic focus.
  final String? shapeId;

  /// Optional source vertex/edge index associated with [shapeId].
  final int? elementIndex;

  /// Stable owner-local item identity, such as a Level section ID.
  final String? elementId;

  /// Authored field associated with this finding, without parsing its message.
  final String? fieldKey;

  /// Operations rejected by this finding when its severity is [ValidationSeverity.error].
  ///
  /// Existing errors block every operation by default. Runtime-readiness errors
  /// may exclude Save without being downgraded or hidden from diagnostics.
  final Set<AuthoringOperation> blockingOperations;

  /// Whether this error prevents [operation]; warnings and information never do.
  bool blocks(AuthoringOperation operation) =>
      severity == ValidationSeverity.error &&
      blockingOperations.contains(operation);
}

/// Human-readable export byproduct (summary, diff, diagnostics, etc.).
@immutable
class ExportArtifact {
  const ExportArtifact({required this.title, required this.content});

  final String title;
  final String content;
}

/// Typed outcome of a plugin export attempt.
enum ExportOutcome {
  /// Repository outputs were written and verified normally.
  applied,

  /// The candidate required no repository writes.
  noChanges,

  /// Candidate validation rejected the write before persistence began.
  validationFailed,

  /// Optimistic concurrency rejected changed source bytes.
  sourceDrift,

  /// Planning or persistence rejected the write and rollback completed.
  failed,

  /// Persistence failed and transaction recovery could not restore everything.
  rollbackIncomplete,

  /// Outputs were committed, but transaction-owned recovery cleanup remains.
  appliedWithCleanupRequired,
}

extension ExportOutcomeClassification on ExportOutcome {
  /// Whether the outcome requires immediate failure feedback to the author.
  bool get isFailure => switch (this) {
    ExportOutcome.validationFailed ||
    ExportOutcome.sourceDrift ||
    ExportOutcome.failed ||
    ExportOutcome.rollbackIncomplete => true,
    ExportOutcome.applied ||
    ExportOutcome.noChanges ||
    ExportOutcome.appliedWithCleanupRequired => false,
  };
}

/// Result of a plugin export operation.
///
/// Legacy plugins may continue to provide only [applied]; [outcome] then
/// resolves to [ExportOutcome.applied] or [ExportOutcome.noChanges]. Plugins
/// that convert expected failures into results must set a failure-classified
/// outcome and provide [message] so session/UI code can surface failure without
/// parsing artifact names.
@immutable
class ExportResult {
  ExportResult({
    required this.applied,
    ExportOutcome? outcome,
    this.message,
    this.recovery,
    List<ExportArtifact> artifacts = const <ExportArtifact>[],
  }) : outcome =
           outcome ??
           (applied ? ExportOutcome.applied : ExportOutcome.noChanges),
       artifacts = List<ExportArtifact>.unmodifiable(artifacts) {
    if (applied !=
        (this.outcome == ExportOutcome.applied ||
            this.outcome == ExportOutcome.appliedWithCleanupRequired)) {
      throw ArgumentError(
        'ExportResult.applied must agree with the typed export outcome.',
      );
    }
    if (this.outcome.isFailure &&
        (message == null || message!.trim().isEmpty)) {
      throw ArgumentError(
        'Failed export results require an actionable message.',
      );
    }
  }

  final bool applied;
  final ExportOutcome outcome;
  final String? message;
  final AuthoringExportRecovery? recovery;
  final List<ExportArtifact> artifacts;
}

/// A transaction's bounded, verified recovery action; never another export.
abstract interface class AuthoringExportRecovery {
  Future<ExportResult> retry();
}

/// Expected typed failure surfaced without losing the transaction outcome.
abstract interface class AuthoringExportFailure implements Exception {
  ExportResult get exportResult;
}

/// Optimistic-concurrency rejection shared by source stores and the session.
class AuthoringSourceDrift extends StateError
    implements AuthoringExportFailure {
  AuthoringSourceDrift(super.message);

  @override
  ExportResult get exportResult => ExportResult(
    applied: false,
    outcome: ExportOutcome.sourceDrift,
    message: message,
  );
}

/// One pending file change preview generated by a plugin.
///
/// [relativePath] is workspace-relative and [unifiedDiff] is display-oriented
/// text, not a guaranteed apply-ready patch format.
@immutable
class PendingFileDiff {
  const PendingFileDiff({
    required this.relativePath,
    required this.editCount,
    required this.unifiedDiff,
  });

  final String relativePath;
  final int editCount;
  final String unifiedDiff;
}

/// Plugin-reported pending-change summary for unsaved edits.
///
/// [changedItemIds] contains domain-defined opaque ids for changed authoring
/// objects when the UI needs per-item dirty state. [fileDiffs] describes the
/// pending file writes. A domain may populate one or both collections depending
/// on the workflow it supports.
@immutable
class PendingChanges {
  PendingChanges({
    List<String> changedItemIds = const <String>[],
    List<PendingFileDiff> fileDiffs = const <PendingFileDiff>[],
  }) : changedItemIds = List<String>.unmodifiable(changedItemIds),
       fileDiffs = List<PendingFileDiff>.unmodifiable(fileDiffs);

  static const PendingChanges empty = PendingChanges._empty();

  const PendingChanges._empty()
    : changedItemIds = const <String>[],
      fileDiffs = const <PendingFileDiff>[];

  final List<String> changedItemIds;
  final List<PendingFileDiff> fileDiffs;

  /// Fast unsaved-changes gate used by session/UI.
  ///
  /// Checks both item ids and file diffs so this remains correct even if one
  /// side is accidentally left empty by a plugin implementation.
  bool get hasChanges => changedItemIds.isNotEmpty || fileDiffs.isNotEmpty;
}

/// Domain adapter contract between editor session orchestration and a concrete
/// authoring implementation (entities/prefabs/chunks).
abstract class AuthoringDomainPlugin {
  /// Stable unique plugin identifier used for route/session selection.
  String get id;

  /// Loads domain data from repository files under [workspace].
  Future<AuthoringDocument> loadFromRepo(EditorWorkspace workspace);

  /// Validates [document] and returns deterministic issue ordering.
  List<ValidationIssue> validate(AuthoringDocument document);

  /// Builds the UI-editable scene projection for [document].
  EditableScene buildEditableScene(AuthoringDocument document);

  /// Applies one edit command to [document].
  ///
  /// Return the original instance when command is a no-op so session-level
  /// undo/redo does not record unnecessary history entries.
  AuthoringDocument applyEdit(
    AuthoringDocument document,
    AuthoringCommand command,
  );

  /// Exports [document] to repository files.
  ///
  /// Returns [ExportResult.applied] false when no file writes were needed.
  Future<ExportResult> exportToRepo(
    EditorWorkspace workspace, {
    required AuthoringDocument document,
  });

  /// Describes file/item deltas for [document] against repository baseline.
  PendingChanges describePendingChanges(
    EditorWorkspace workspace, {
    required AuthoringDocument document,
  });
}
