...
class PlayerCharacterDefinition {
  ...
  void assertValid() {
    assert(() {
      ...
      final anchor = renderAnim.anchorPoint;
      if (!anchor.x.isFinite || !anchor.y.isFinite) {
        throw StateError(
          'PlayerCharacterDefinition($id) renderAnim.anchorPoint must be finite (got $anchor)',
        );
      }
      if (anchor.x < 0 ||
        ...
        throw StateError(
          'PlayerCharacterDefinition($id) renderAnim.anchorPoint must be within the frame '
