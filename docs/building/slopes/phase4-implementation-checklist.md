...
| Explicit collinear normalization existed only for committed shapes. A rejected aligned draft or drag retained its preview as designed, but active-operation guards then made Normalize and tool chips appear unresponsive while Apply correctly remained blocked. | Keep collinear source invalid until an explicit decision. Let Normalize validate, normalize, and commit the visible draft/gesture as one history edit; let selecting a different tool discard the uncommitted operation and switch. Preserve all other blocking diagnostics and owner policy checks. | Both Prefab and Chunk recover without reloading or weakening Core geometry. Apply remains unavailable until the preview is normalized or discarded, and the manual pass exercises aligned recovery in both routes. |
