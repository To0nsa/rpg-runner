...
| A normal authoring record must preserve malformed/noncanonical input for diagnostics, while a migration target must emit deterministic canonical bytes. | `PrefabV3Def` snapshots supplied shape/tag order without rewriting it; `PrefabV3TargetDocument` canonicalizes copied records only at the target boundary. | The future strict v3 store can diagnose source order before save, while migration output and revision comparisons stay deterministic. |
