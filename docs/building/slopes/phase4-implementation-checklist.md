...
the Chunk, while expanded-shape failures own the referenced Prefab. The staged
seam validator now uses the same blocking envelope: duplicate/case-colliding,
missing, and wrong-level findings own each offending Chunk; a directed physical
...
The composed staged-output gate now also adapts that plan's exact missing,
stale, unexpected, or unreadable findings into the same blocking envelope,
...
- [x] Compose retained prefab-owner controls over the explicit v3 staging
      ...
      current-schema source apply is explicit and confirmed.
...
- [x] Add an explicit store/plugin loader that strictly
      ...
      selects it for already-current source; checked-in v2 remains legacy.
- [x] Add a prefab-v3 workspace with owner-local
...
- [x] Stage retained prefab/tile slice and platform-module operations behind
      ...
      pending diffs apply only against their strict current-source baselines.
...
- [x] Add a chunk-v2 workspace with active-level
      ...
      text, owner diagnostics, session history, guarded reload, and confirmed
      atomic source apply for changed current documents.
...
2. **Normal validation and pending-change parity.**
   ...
   - Every changed Prefab-v3 and Chunk-v2 candidate now crosses its
     ...
     Export validates before its clean/no-op result or changed-source apply,
     so invalid source cannot masquerade as a successful no-op.
3. **Store/export parity without repository migration.**
   ...
   - `ChunkStore` builds a deterministic v2 plan and pending diff for
     ...
     reloads byte-identically. Normal plugin export now uses this path only
     after strict current-schema loading.
   - `PrefabStore` now builds the exact paired prefab-v3/tile-v2 plan from
     ...
     all-current fixtures. Normal plugin export now uses this path only after
     strict current-schema loading; migration writes remain locked.
4. **Route replacement.**
   ...
   - Prefab-v3 authoring composes owner create/edit/duplicate/rename/delete
     ...
     and undo/redo. The retained visual atlas and
   ...
   - Chunk-v2 authoring composes active-level owner selection, metadata/status/
     ...
     command. Owning-prefab navigation now atomically loads the explicit
     Prefab-v3 document and selects the requested stable owner. Normal loading
     selects this route for a complete strict v2 source set; reload and
     confirmed source apply use the normal session/store path.
5. **Current-schema normal-loader proof.**
   - [x] In complete temporary v3/v2 fixtures, normal plugin loading—not an
...
- [x] load/validate/edit/pending diff/export for prefab v3 and chunk v2
- [x] atomic paired prefab/tile writes and one-file chunk writes
- [x] route/session/workspace switching with polygon draft state
- [x] create/duplicate/rename/deprecate revision semantics in Prefab-v3 and
      Chunk-v2 documents
...
| The first manual usability attempt found that every completed pointer gesture silently reset the active edit tool to Select, so moving a second vertex appeared unresponsive unless Move vertex was chosen again. | Preserve the gesture-owned Move vertex, Insert vertex, or Move shape tool across accepted and no-op gesture commits. Keep polygon close and explicit Escape cancellation as mode exits. Require both Prefab and Chunk surface tests to start a second gesture on a different vertex without reselecting the tool. | Consecutive polygon edits now follow normal persistent-tool behavior through the shared reducer. Manual acceptance must repeat the exact sequence; the final human gate remains open until that rerun passes. |
| Explicit collinear normalization existed only for committed shapes. A rejected aligned draft or drag retained its preview as designed, but active-operation guards then made its recovery controls appear inert while Apply correctly remained blocked. | Keep collinear source invalid until an explicit decision. A draft Save normalizes and validates before its single commit. Normalize remains explicit for a rejected committed-shape gesture, where selecting another tool may instead discard only that preview. Preserve all other blocking diagnostics and owner policy checks. | Both Prefab and Chunk recover without reloading or weakening Core geometry. Apply remains unavailable until local work is saved, normalized, or discarded, and the manual pass exercises both draft and saved-shape recovery. |
| Polygon creation originally treated the whole open draft as one cancellable operation, so Undo discarded it and the committed-shape Move/Insert tools could not correct points before close. Separate Close Draft and Normalize Draft actions also exposed reducer mechanics rather than the authoring intent. | Make creation one local draft session with immutable vertex-snapshot Undo/Redo for append, move, and open-edge insert. Permit only Move vertex and Insert vertex during that session; disable Select, Create, and Move shape. Replace both completion actions with Save, which normalizes, validates, and dispatches exactly one source commit. | A badly placed point can be corrected without restarting or polluting source history. Prefab and Chunk share the reducer, hit testing, keyboard behavior, responsive tool gating, and one-commit Save boundary. |
...
| 2026-08-11 / `d7c5237e` | Persistent polygon edit tools after manual finding | Flutter test VM and Dart analyzer on Windows with Docker running | The first manual Prefab interaction exposed a silent post-gesture reset to Select. The shared reducer now retains Move vertex, Insert vertex, and Move shape after accepted and no-op commits. Direct reducer coverage locks all three modes, and Prefab/Chunk surface tests move one vertex before immediately beginning a gesture on a different vertex with no tool reselection. Editor analysis is clean, all 31 focused interaction/controller tests pass, and the low-memory `--concurrency=1 --fail-fast` full suite passes all 359 editor tests. The initial full-suite command was stopped only by its two-minute command allowance; the repeated longer run completed in 154 seconds. The manual acceptance checklist now requires this exact sequence in both routes, and its human pass remains pending. |
| 2026-08-12 / `a8ee747f` + `beaf6b23` + `9f88f5f0` | Aligned-vertex recovery from drafts and gestures | Flutter test VM and Dart analyzer on Windows with Docker running | Manual use exposed that a correct `collinear_middle_vertex` rejection retained its preview while active-operation guards made Normalize and tool changes appear inert. Normalize now explicitly validates and commits the visible rejected gesture or draft after Core removes redundant middle vertices; a draft normalization also closes the polygon. Selecting another tool discards only the uncommitted operation and activates that tool. Apply remains blocked until recovery. Both routes expose a narrow-width-safe **Normalize draft** action. Editor analysis is clean, all 47 focused reducer/controller/workspace tests pass, and the low-memory `--concurrency=1 --fail-fast` full suite passes all 363 editor tests in 164 seconds. The manual pass now exercises both normalization and discard recovery in Prefab and Chunk; its human gate remains pending. |
| 2026-08-12 / `72c75d48` | Editable polygon construction sessions | Flutter test VM and Dart analyzer on Windows with Docker running | The reviewed usability model supersedes the intermediate separate Close/Normalize Draft controls with one **Save** action. Append, draft-vertex move, and open-edge insert each add an immutable local vertex snapshot; local Undo/Redo never changes the session document, owner revision, pending source changes, or source-history cursor. Save performs explicit collinear normalization, complete Core/owner validation, and exactly one source commit. During a draft, only Move vertex and Insert vertex are enabled; Select, Create, and Move shape are disabled. Shared draft projection and open-edge hit testing drive real pointer behavior in both routes, including narrow layouts. Editor analysis is clean, all 66 focused reducer/projection/painter/controller/workspace tests pass, and the low-memory `--concurrency=1 --fail-fast` full suite passes all 369 editor tests in 164 seconds. The manual human gate remains pending on this revision. |
