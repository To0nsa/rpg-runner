...
the transition record in its message. Migration and the remaining editor-wide
validation paths still need this boundary before the broad diagnostic gate can
close.
...
either semantic or byte validation fails. Migration and the remaining
editor-wide/export paths are still outside the universal boundary.
...
because migration, remaining editor domains, and normal
