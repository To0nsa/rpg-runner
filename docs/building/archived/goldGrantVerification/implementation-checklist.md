...
> Archived: this checklist describes the prior client/read-time reconciliation
> approach. Do not use it to guide new implementation work.

This checklist turns [plan.md](plan.md) into an execution sequence.

Audit note, July 7, 2026: backend lifecycle/projection/settlement work and the
Game Over reward row appear implemented, but persistent gold screens do not
match the documented verified-only display rule. Current hub, town, and profile
tests assert that unverified reward gold is included in global totals.

Use [strategy-refresh.md](../../goldGrantVerification/strategy-refresh.md) as the next implementation
strategy. The remaining work is not a pending-currency UI pass; it is a
single-wallet verification latency and settlement synchronization fix.
...
- [x] Re-read [plan.md](plan.md) and confirm state names/contract fields before editing code.
- [x] Cross-check hybrid write assumptions in [ownershipHybridWrite/plan.md](../ownershipHybridWrite/plan.md).
...
- [x] Projection input sources:
	- [x] `run_session`
	- [x] `reward_grant`
	- [x] `validated_run`
- [x] Projection output fields:
	- [x] `status`
	- [x] `provisionalGold`
...
- [ ] Verify hub/town/profile continue showing verified-only gold.
	- [ ] Current code routes these screens through `AppState.displayGold`, which includes `unverifiedGold`; patch screens/tests back to verified-only wallet display.
...
- [ ] Add tests ensuring hub/town/profile remain verified-only while Game Over shows provisional reward.
	- [ ] Current hub/town/profile tests assert the opposite behavior and need to be corrected if this plan's invariant still stands.
...
- [ ] Hub/town/profile show verified-only gold while provisional reward remains scoped to reward-context UI.
