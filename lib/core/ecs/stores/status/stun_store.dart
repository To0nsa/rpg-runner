...
class StunDef {
  const StunDef({required this.ticksLeft, required this.magnitude});

  final int ticksLeft;
  final double magnitude;
}

/// Active stun status (stun for duration).
class StunStore extends SparseSet {
  final List<int> ticksLeft = <int>[];
  final List<double> magnitude = <double>[];

  void add(EntityId entity, StunDef def) {
